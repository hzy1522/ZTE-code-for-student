...
6. 添加完成后立即点击"结算"或"提交订单"按钮（y>850）
7. 【重要】点击结算/提交订单后，任务即完成，立即输出 COMPLETE
...
【何时完成】
- 点击"结算"或"提交订单"按钮后，任务即完成，立即输出 COMPLETE
- 【禁止】继续点击"支付"、"付款"等按钮，任务流程在提交订单时结束
- 【安全原则】Agent 只负责操作到"提交/确认"为止，不完成实际支付
""",
    TaskType.TRANSPORT: """
## 打车/交通类任务规则
【关键流程】
1. 百度地图打车界面有上下两个输入框：【上方是出发地，下方是目的地】
2. 先点击上方输入框（出发地），输入或选择出发地
3. 再点击下方输入框（目的地），输入或选择目的地
4. 【重要】如果任务说"地址选项都选第一个"，则输入后【必须】点击下拉列表的第一个选项

【禁止】
- 不要默认当前位置，必须按照任务指定的地点操作
- 不要先输入目的地再输入出发地，必须按顺序
- 【重要】不要点击"立即叫车"、"确认打车"等按钮，任务在选择完地址后结束

【何时完成】
- 如果任务只要求输入地址并选择选项，则在选择完地址后立即输出 COMPLETE
- 如果任务要求"打车"到某地，则输入完地址后立即输出 COMPLETE（不执行实际叫车）
- 【核心原则】任务目标是"操作到选择地址"，不是"完成叫车"

【UI状态注意】
- 输入地址后，下方会出现下拉列表
- 【必须】点击列表第一项，不能只输入文字就结束
- 点击列表项后会跳转到确认页面
""",
}
...
5. 打开应用时应用名与图标文字完全一致
6. 【安全红线】绝对不点击"支付"、"付款"、"立即支付"等按钮，不完成实际支付
